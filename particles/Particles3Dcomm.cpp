...
void c_vDist::add(double x, double y, double z, double u, double v, double w) {
    ...
    double r2 = (x - vDistLoc_x) * (x - vDistLoc_x) + (y - vDistLoc_y) * (y - vDistLoc_y) +
                (z - vDistLoc_z) * (z - vDistLoc_z);
...
void Particles3Dcomm::interpP2G(Field *EMf, Grid *grid, VirtualTopology3D *vct) {
    ...
    //#pragma omp parallel
    {
        ...
        for (register long long i = 0; i < nop; i++) {
            ...
            if (relativistic) {
                Matrix3d tensor =
                        evaluateAlphaRotationTensor(grid, EMf, i) *
                        (2.0 * M_PI * qom * q[i] * dt * dt * EMf->getTheta());
                for (int ii = 0; ii < 2; ii++)
                    for (int jj = 0; jj < 2; jj++)
                        for (int kk = 0; kk < 2; kk++) {
...
/** return the relativistic Kinetic energy */
double Particles3Dcomm::getRelativisticKe() {
    double localKe = 0.0;
    double totalKe = 0.0;
    for (register long long i = 0; i < nop; i++) {
...
void Particles3Dcomm::Print(VirtualTopology3D *ptVCT) const {
    ...
    for (long long i = 0; i < nop; i++)
        cout << "Particles #" << i << " x=" << x[i] << " y=" << y[i] << " z=" << z[i] << " u=" << u[i] << " v=" << v[i]
             << " w=" << w[i] << endl;
    cout << endl;
}
...
void Particles3Dcomm::get_weights(Grid *grid, double xp, double yp, double zp, int &ix, int &iy, int &iz,
...
void
Particles3Dcomm::get_El(const double weights[2][2][2], int ix, int iy, int iz, double &Exl, double &Eyl, double &Ezl,
...
void
Particles3Dcomm::get_Bl(const double weights[2][2][2], int ix, int iy, int iz, double &Bxl, double &Byl, double &Bzl,
                        double ***Bx, double ***By, double ***Bz, double ***Bx_ext, double ***By_ext, double ***Bz_ext,
                        double Fext) {
